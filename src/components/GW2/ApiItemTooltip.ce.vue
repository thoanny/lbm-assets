...
<template>
    <div>
        <img :src="props.item.icon" alt=""
            :class="['border-gw2-rarity-' + props.item.rarity, 'w-16 h-16 border border-2 rounded']" v-if="props.item.icon"
            v-tippy>
                ...
                <div v-else class="flex flex-col gap-2 text-md">
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <span
                                v-if="itemTooltipData.details?.type && ['Default', 'Generic'].indexOf(itemTooltipData.details?.type) < 0">
                                ({{ i18n('Details.Type.' + itemTooltipData.details?.type, fr) }})
                            </span>
                        </div>
                    </div>

                    <div v-if="itemTooltipData.flags.length > 0" class="flex flex-wrap gap-1 text-xs text-white opacity-60">
