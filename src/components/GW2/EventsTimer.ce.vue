...
const eventModal = ref({ isActive: false, row: {}, event: {} });
    ...
    <div class="lbm-et" v-if="categories">
        <div class="lbm-et__hscroll" ref="el">
            <div
                ...
                :key="category.uid"
                v-show="!isCategoryHidden(category.uid)"
            >
                <div
                    class="lbm-et__category"
                    :style="{ width: WIDTH * 24 * 60 + 'px' }"
                    v-if="settings.showCategoryName"
                >
                    <span class="sticky left-2 bangers">{{ category.name }}</span>
                </div>
                <div
                    class="lbm-et__row flex flex-col"
                    v-for="row in category.rows"
                    :key="row.uid"
                    v-show="!isRowHidden(row.uid)"
                >
                    <div
                        class="lbm-et__row__name"
                        :style="{ width: WIDTH * 24 * 60 + 'px' }"
                    ...
                    <div class="lbm-et__events">
                        ...
                        >
                            <div
                                v-text="convertMinutesToText(e.start)"
                                v-if="settings.showEventHour"
                            ></div>
                            <div
                                class="truncate font-bold"
                                :class="{
                                    'line-through italic opacity-50': isEventCompleted(e.id),
                                }"
                            >
                                {{ e.name }}
                            </div>
            ...
            <div class="lbm-et__cursor" :style="{ left: cursorPosition }">
                <span
                    class="py-1 px-2 mt-2 text-xs inline-block"
                    style="transform: translateX(calc(-50% + 1px)); background: red"
                    v-text="currentTime"
                ></span>
            </div>
        </div>
        <div class="lbm-et__toolbar absolute top-0 right-0" v-if="!props.hideToolbar">
            <div class="flex gap-1 bg-neutral rounded-bl-xl p-1">
                ...
                <button
                    class="lbm-btn lbm-btn-square lbm-btn-sm"
                    :class="{ 'lbm-btn-success': settings.isCentered }"
                ...
                <label
                    for="modalVisibility"
                    class="lbm-btn lbm-btn-square lbm-btn-sm lbm-btn-primary"
                    @click="false"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="lbm-modal">
                    <div class="lbm-modal-box">
                            ...
                            <li v-for="category in categories" :key="category.uid">
                                <a @click.prevent="toggleHideCategory(category.uid)">
                                    <svg
                                        ...
                                        class="h-4 w-4"
                                        fill="currentColor"
                                        v-if="isCategoryHidden(category.uid)"
                                    >
                                        <path
                        ...
                        <label for="modalVisibility" class="lbm-btn lbm-btn-primary lbm-btn-sm mt-4"
                            >Fermer</label
                        >
                    </div>
                    ...
                    <div class="lbm-modal-box">
                        ...
                        <div class="mb-2">
                            <label class="cursor-pointer flex items-center justify-start gap-2">
                                <input
                                    type="checkbox"
                                    class="lbm-toggle lbm-toggle-sm lbm-toggle-success"
                        ...
                        <div class="mb-3">
                            <label class="cursor-pointer flex items-center justify-start gap-2">
                                <input
                                    type="checkbox"
                                    class="lbm-toggle lbm-toggle-sm lbm-toggle-success"
                                    v-model="settings.showEventHour"
                                    @change="saveSettings"
                                />
                        ...
                        <div>
                            <button
                                class="lbm-btn lbm-btn-sm text-xs"
                                @click.prevent="resetEventsCompleted"
                            >
                                Réinitialiser le suivi des événements
                            </button>
                        </div>

                        <label for="modalSettings" class="lbm-btn lbm-btn-primary lbm-btn-sm mt-4"
                            >Fermer</label
                        >
                    </div>
        ...
        <input
            type="checkbox"
            id="modalEvent"
            class="lbm-modal-toggle"
            v-model="eventModal.isActive"
        />
        <div class="lbm-modal lbm-modal-bottom sm:lbm-modal-middle">
            <div class="lbm-modal-box">
                ...
                <h4
                    class="font-bold italic -mt-5 mb-6 opacity-50 text-center"
                    v-if="eventModal.row.name"
                >
                    {{ eventModal.row.name }}
                </h4>
                <a
                    :href="eventModal.event.link"
                    target="_blank"
                    class="lbm-btn lbm-btn-block mb-2"
                ...
                <button
                    class="lbm-btn lbm-btn-block mb-2"
                    v-if="eventModal.event.chatlink"
                    @click.prevent="
                        copyToClipboard(eventModal.event.name, eventModal.event.chatlink)
                    "
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                ...
                <button class="lbm-btn lbm-btn-block mb-2" @click.prevent="handleEventComplete">
                    <svg
                        ...
                        class="h-5 w-5"
                        fill="currentColor"
                        v-if="!isEventCompleted(eventModal.event.id)"
                    >
                        <path
                    ...
                    {{
                        isEventCompleted(eventModal.event.id)
                            ? 'Marquer comme non terminé'
                            : 'Marquer comme terminé'
                    }}
                </button>
                <label for="modalEvent" class="lbm-btn lbm-btn-primary mt-4 lbm-btn-block"
                    >Fermer</label
                >
            </div>
